...
module.exports = async () => {
  ...
  try {
    //Peticion a axios para traer todos los tipos de perros desde 'dogapi.co'
    const response = (await axios(`https://api.thedogapi.com/v1/breeds?api_key=${API_KEY}`)).data;
    
    const filterTemp = response.map(t =>{
      return t.temperament ? t.temperament.split(", ") : []  // recorre los temperamentos y se separan por comas, de lo contrario es array vacio
